...
export default (callback) => {
  updater.on('available', (task) => {
    if (!updaterWindow) {
      ...
      // 加载更新提示界面
      updaterWindow.loadURL(`data:text/html, ${html}`)
      webContents = updaterWindow.webContents
    }
  })
  updater.on('not-available', (task) => webContents && webContents.send('update_message', `『${task.name}』无需更新！`))
  updater.on('progress', (task, p) => webContents.send('update_progress', p))
  updater.on('downloaded', (task) => webContents.send('update_message', `『${task.name}』更新完成！`))
  updater.on('completed', (result) => {
    if (result) {
      webContents && webContents.send('update_message', '全部更新完成！正在重启')
      // return updater.quitAndInstall(2000)
    }
    callback()
  })
  updater.on('error', (error) => {
    webContents.send('update_message', '更新出错，请联系作者！')
    callback()
  })
